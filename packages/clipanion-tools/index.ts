...
import {type FSWatcher, type Stats, statSync, watch} from 'fs';
...
export class ClipanionBinary {
  ...
  private commandSpecs: Promise<Array<CommandSpec> | null>;

  private lastStat: Stats;
  private updateCallbacks = new Set<() => void | Promise<void>>();
  private watcher: FSWatcher | null = null;

  constructor(binPath: string, {enableWatch = true, specCommand = [`--clipanion-commands`]}: Options = {}) {
    this.binPath = binPath;

    this.lastStat = statSync(this.binPath);
    this.commandSpecs = this.query<Array<CommandSpec>>(specCommand);

    if (enableWatch) {
      this.watcher = watch(this.binPath, async event => {
        const oldStat = this.lastStat;
        const newStat = this.lastStat = statSync(this.binPath);
  ...
  async onUpdate(fn: () => void | Promise<void>) {
